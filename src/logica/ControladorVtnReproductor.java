 ...
 * up.
 */
public class ControladorVtnReproductor implements Initializable {
	...
	public void initialize(URL url, ResourceBundle resourceBundle) {
		...
		// To create a media player you need to implement the structure of the 3 nested
		// media objects,
		// media, media player, and media view.
		// The media player wraps the media and the media view wraps the media player.
		// mediaVideo = new Media(new
		// File("src/recursos/hello-world.mp4").toURI().toString());
		...
		Image imagePlay = new Image(getClass().getResource("/recursos/play-btn.png").toString());
		...
		Image imageStop = new Image((getClass().getResource("/recursos/stop-btn.png").toString()));
		...
		//Image imageRestart = new Image(new File("src/recursos/restart-btn.png").toURI().toString());
		Image imageRestart = new Image((getClass().getResource("/recursos/restart-btn.png").toString()));
		...
		//(getClass().getResource("src/recursos/stop-btn.png").toString())
		Image imageVol = new Image((getClass().getResource("/recursos/volume.png").toString()));
		...
		//(getClass().getResource("src/recursos/stop-btn.png").toString())
		Image imageFull = new Image((getClass().getResource("/recursos/fullscreen.png").toString()));
		...
		Image imageMute = new Image((getClass().getResource("/recursos/mute.png").toString()));
		...
		Image imageExit = new Image((getClass().getResource("/recursos/exitscreen.png").toString()));
		ivExit = new ImageView(imageExit);
		ivExit.setFitHeight(35);
		ivExit.setFitWidth(35);

		// Imagenes volver
		//Image imgVolver = new Image(new File("src/recursos/flechaizq.png").toURI().toString());
		Image imgVolver = new Image((getClass().getResource("/recursos/flechaizq.png").toString()));
		ivVolver = new ImageView(imgVolver);
		ivVolver.setFitWidth(35);
		ivVolver.setFitHeight(35);
		...
		// Colocarle la imagen al btn volver

		volver.setGraphic(ivVolver);

		// We want to start out with the slider for the volume removed because we want
		// it to appear
		...
		volver.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent actionEvent) {
				 ...
				 * 
				 * @Override public void run() { Main.loginAceptado(); } });
				 */

				/*
				 * FXMLLoader loader = new FXMLLoader();
				 * loader.setLocation(getClass().getResource("orden.fxml"));
				 * 
				 * Stage stage = new Stage(); VBox mainPane; try { mainPane = (VBox)
				 * loader.load(); Scene scene = new Scene(mainPane);
				 * 
				 * stage.setScene(scene); stage.initStyle(StageStyle.DECORATED);
				 * stage.setResizable(false); } catch (IOException e) { // TODO Auto-generated
				...
				Scene sceneantigua = volver.getScene();
				mpVideo.stop();
				/*
				 * Stage newStage = new Stage(); newStage.setTitle("Login");
				 */
				/*
				VBox mainPane = new VBox();
				Button a = new Button("Ver");
				Label label = new Label();
				// Creating a graphic (image)
				//System.out.println("ES ES " + (Main.getPeliRepro().getDirThumbnail()));
				Image img = new Image((getClass().getResourceAsStream(Main.getPeliRepro().getDirThumbnail())));
				ImageView view = new ImageView(img);
				view.setFitHeight(500);
				view.setPreserveRatio(true);
				label.setGraphic(view);
				a.setOnAction(e -> {
					s.setScene(sceneantigua);
					mediaVideo = new Media(Main.getPeliRepro().getFile().toString());
					mpVideo = new MediaPlayer(mediaVideo);
					mvVideo.setMediaPlayer(mpVideo);
					mpVideo.play();
				});
				mainPane.getChildren().addAll(a, label);
				...
				*/
				//COSAS NUEVAS
		    	  ...
		    	  //s.setScene(Main.prueba);
		    	  Scene sceneaPrincipal;
				try {
					sceneaPrincipal = new Scene(FXMLLoader.load(getClass().getClassLoader().getResource("Vistas/VistaVtnPrincipal.fxml")), 800, 600);
					stagePrincipal.setScene(sceneaPrincipal);
				...
				/* PARA HACER LA IDEA DE QUE APAREZCA UNA VEZNTANA INTERMEDIA HACER EL S.CLOSE
				 * 
				 * 
				 * OTRA IDEA BUENA ES PONER EL BTN ACTUALIZAR EN EL MISMA VTN Y QUE EL REPRODUCTOR NO DESAPAREZCA NOMAS
				 * 
				 * 
				 * 
				 * 
				 ...
				 * SwingUtilities.invokeLater(new Runnable() {
				 * 
				 * @Override public void run() { System.out.println("lala");
				 * Main.loginAceptado(); } });
				 */
				...
				/*
				 * VistaVtnPrincipal v = new VistaVtnPrincipal(); //v.setVisible(atEndOfVideo);
				 * 
				 * v.setVisible(true);
				 */
	...
	/*
	 * public static void cambiar() { Stage s = (Stage)
	 * volver.getScene().getWindow(); Scene sceneantigua = volver.getScene();
	 * mpVideo.stop();
	 * 
	 * VBox mainPane = new VBox(); Button a = new Button("recargar");
	 * a.setOnAction(e -> { s.setScene(sceneantigua); mediaVideo = new
	 * Media(Main.getPeliRepro().getFile().toURI().toString()); mpVideo = new
	 * MediaPlayer(mediaVideo); mvVideo.setMediaPlayer(mpVideo); mpVideo.play(); });
	 * mainPane.getChildren().add(a); Scene scene = new Scene(mainPane, 200, 200);
	 * 
	 * 
	 * s.setScene(scene); }
	 */
